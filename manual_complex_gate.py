"""Nonlinear router for run_moe_gate_cycle.py --gate-source."""
...
class LLMGeneratedGate(nn.Module):
    def __init__(self, model_dim: int, num_experts: int):
        ...
        self.correction = nn.Linear(width, num_experts, bias=False)
        # Preserve copied native logits while allowing the correction to learn.
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        return self.base(x) + self.correction(self.activation(features))
