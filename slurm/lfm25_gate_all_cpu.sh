...
#SBATCH --job-name=lfm25-gate-all
#SBATCH --output=slurm-lfm25-gate-all-%j.out
#SBATCH --error=slurm-lfm25-gate-all-%j.out
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
LOCAL_ROOT="${TMPDIR:-/tmp}/lfm25_gate_all_${SLURM_JOB_ID}"
VENV_DIR="${LOCAL_ROOT}/venv"
OUTPUT_DIR="${OUTPUT_DIR:-${DATA_ROOT}/outputs/lfm25_gate_all_${SLURM_JOB_ID}}"
